...
describe('ProductionErrorBoundary', () => {
	...
	it('renders production-friendly error UI when error occurs', () => {
		const ThrowError = () => {
		...
		expect(boundary).toBeTruthy()
		expect(boundary?.textContent).toContain('Something went wrong')
		expect(boundary?.textContent).toContain('Please refresh the page')
		expect(boundary?.querySelector('details')).toBeFalsy()
	})
})
