...
function cached<T, O extends object>(factory: (locale: string, options: O) => T): (locale: string, options: O) => T {
...
export const cachedRelativeTimeFormat = cached(
	(locale: string, options: Intl.RelativeTimeFormatOptions) => new Intl.RelativeTimeFormat(locale, options),
)
