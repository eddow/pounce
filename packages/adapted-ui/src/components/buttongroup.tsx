...
function findNextFocusableOutsideGroup(
	...
	if (backwards) {
		return (
			preferredBefore.at(-1)?.element ??
			before.at(-1)?.element ??
			preferredAfter.at(-1)?.element ??
			after.at(-1)?.element ??
			null
		)
	}
	return (
		preferredAfter[0]?.element ??
		after[0]?.element ??
		preferredBefore[0]?.element ??
		before[0]?.element ??
		null
	)
}
...
function handleButtonGroupNavigation(
	...
	const enabledButtons = Array.from(
		groupContainer.querySelectorAll<HTMLButtonElement>(selector)
	).filter((btn) => !btn.disabled)
	if (enabledButtons.length === 0) return

	const isVertical =
		groupContainer.classList.contains('pounce-buttongroup-vertical') ||
		groupContainer.classList.contains('pounce-toolbar-vertical')
