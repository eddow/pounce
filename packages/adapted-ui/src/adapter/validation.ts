 ...
 */
export function validateAdapter(adapter: Partial<FrameworkAdapter>): void {
	...
	if (result instanceof type.errors) {
		...
		throw new Error(
			`[pounce/ui] Invalid adapter configuration:\n${errorMessages}\n\n` +
 ...
 */
export function validateComponentAdapter(componentName: string, adapter: any): void {
	if (process.env.NODE_ENV === 'production') {
		return
	}
	...
	if (result instanceof type.errors) {
		...
		console.error(
			`[pounce/ui] Invalid adapter configuration for "${componentName}":\n${errorMessages}\n\n` +
