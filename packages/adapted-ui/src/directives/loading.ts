...
export function loading(target: Node | Node[], value: boolean) {
	...
	if (value) {
		...
		if (formElements.has(element.tagName)) (element as HTMLButtonElement).disabled = true
	} else {
		element.removeAttribute('aria-busy')
		element.classList.remove(loadingClass)
		if (formElements.has(element.tagName)) (element as HTMLButtonElement).disabled = false
	}
