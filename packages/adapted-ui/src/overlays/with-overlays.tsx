...
import { applyTransition, getTransitionConfig } from '../shared/transitions'
import type { OverlayEntry, PushOverlayFunction } from './manager'
...
export const WithOverlays = (props: WithOverlaysProps, env: Env) => {
	...
	const layers = props.layers && props.layers.length > 0 ? props.layers.map(renderLayer) : null
	const flat = !layers ? (
	...
	return (
		<fragment>
			...
			>
				<div if={hasBackdrop()} class="pounce-backdrop" onClick={handleBackdropClick} />
				{layers}
				{flat}
			</div>
		</fragment>
