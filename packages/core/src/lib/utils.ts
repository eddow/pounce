...
const defaultsProxy: ProxyHandler<any> & Record<symbol, unknown> = {
	...
	get(target, key) {
		if(typeof key === 'string')
			return (key in target.props ? target.props : target.defs)[key]
	},
	...
	ownKeys(target) {
		if(!target.keys) {
			const keySet = new Set<PropertyKey>([...Reflect.ownKeys(target.props), ...Reflect.ownKeys(target.defs)])
			target.keys = Array.from(keySet).filter(k=> typeof k === 'string')
		}
		return target.keys
	},
	getOwnPropertyDescriptor(target, p) {
		return Reflect.getOwnPropertyDescriptor(target.props, p) || Reflect.getOwnPropertyDescriptor(target.defs, p)
	},
}
/**
...
>(props: P, defs: D): P&D { //Omit<P, keyof D> & { [K in keyof D & keyof P]-?: NonNullable<P[K]> } {
	defs = reactive(defs)
	return new Proxy({props, defs, keys: undefined as undefined | string[]} as any, defaultsProxy) as any
}
