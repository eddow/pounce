...
This guide highlights how Sursaut‑TS (powered by the `mutts` reactivity engine) differs from React. If you’re familiar with React, these notes will help you transfer mental models and avoid common pitfalls.

### Reactivity and State
- **Direct mutation reactivity**: In Sursaut‑TS, reactive state is created with `reactive(...)` and you mutate it directly. No `useState`, no setter functions.
...
- **No Virtual DOM**: Sursaut‑TS updates the real DOM directly with fine‑grained reactivity. There’s no reconciliation against a VDOM tree.
...
In React, changing state triggers a re-render of the component and its subtree. In Sursaut, **component constructors run exactly once** — this is enforced by a **rebuild fence**. If a constructor accidentally reads reactive state directly, the fence blocks re-execution and warns. This is a deliberate design choice, not a limitation:

- React re-renders are expensive (VDOM diffing, reconciliation). Sursaut avoids them entirely.
...
### Env vs Context
- **Env inheritance**: Instead of React Context Providers, Sursaut‑TS uses a prototype‑inherited, reactive `env` object passed to components.
