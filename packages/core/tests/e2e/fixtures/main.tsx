...
async function loadFixtureFixture() {
	...
	console.log('LoadFixture called with hash:', hash)
	...
	try {
		...
		if (!loader) {
			console.error(`Loader not found for ./${hash}Tests.tsx`)
			state.component = <div>Fixture {hash} not found. Available: {window.__fixturesList.join(', ')}</div>
