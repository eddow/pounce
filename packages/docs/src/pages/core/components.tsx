		...
		<article>
			...
			<Section title="Component Signature">
				<p>
					...
					re-renders. The Babel plugin's <code>r()</code> wrappers handle all reactive updates at
					the individual node/attribute level.
				</p>
			...
			<Section title="Render Once">
				<p>
					Because the component body runs inside a rebuild fence, bare reactive reads in the body
					trigger a warning. All reactivity must go through JSX expressions (auto-wrapped by Babel)
					or explicit <code>effect()</code> calls.
				</p>
					...
					props={[
						{
							name: 'produce',
							type: '(env?) => Node | Node[]',
							description: 'Creates real DOM nodes from the element descriptor',
							required: true,
						},
						{
							name: 'render',
							type: '(env?) => Node | Node[]',
							description: 'Cached version of produce — same element always returns same nodes',
							required: true,
						},
						{
							name: 'mount',
							type: '((Node) => cleanup)[]',
							description: 'Callbacks invoked after DOM insertion',
							required: false,
						},
						{
							name: 'condition',
							type: '() => any',
							description: 'Reactive condition for if=/when= directives',
							required: false,
						},
						{
							name: 'tag',
							type: 'string | ComponentFunction',
							description: 'The element tag or component function (for debugging)',
							required: false,
						},
					]}
				/>
