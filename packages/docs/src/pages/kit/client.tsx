...
export default function ClientPage() {
  return (
    <article>
      <h1>Client State</h1>
      <p>
        <code>client</code> is a reactive object that exposes browser state.
        All properties are tracked by mutts — read them in JSX and they update automatically.
      </p>
      ...
      <Section title="URL">
        <p>
          <code>client.url</code> is a reactive parsed URL with pathname, search, hash,
          segments, and query.
        </p>
      ...
      <Section title="API Reference">
        <ApiTable props={[
          { name: 'url', type: 'ClientUrl', description: 'Reactive parsed URL (pathname, search, hash, segments, query)', required: false },
          { name: 'viewport', type: '{ width, height }', description: 'Window inner dimensions', required: false },
          { name: 'online', type: 'boolean', description: 'navigator.onLine', required: false },
          { name: 'prefersDark', type: 'boolean', description: 'prefers-color-scheme: dark', required: false },
          { name: 'focused', type: 'boolean', description: 'document.hasFocus()', required: false },
          { name: 'language', type: 'string', description: 'navigator.language', required: false },
          { name: 'direction', type: "'ltr' | 'rtl'", description: 'Text direction from <html dir>', required: false },
          { name: 'navigate(url)', type: '(string, opts?) => void', description: 'pushState navigation', required: false },
          { name: 'replace(url)', type: '(string, opts?) => void', description: 'replaceState navigation', required: false },
        ]} />
      </Section>
    </article>
  )
