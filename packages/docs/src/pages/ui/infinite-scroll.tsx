...
export default function InfiniteScrollPage() {
	return (
		<article>
			...
			<p>
				Virtualized scrollable list — renders only visible items plus a small buffer. Supports both
				...
				<ApiTable
					props={[
						{ name: 'items', type: 'T[]', description: 'Data array to render', required: true },
						{
							...
							description: 'Fixed row height (px) or estimator function for variable heights',
							required: true,
						},
						{
							name: 'children',
							type: '(item: T, index: number) => JSX.Element',
							description: 'Row renderer — receives item and its index',
							required: true,
						},
						{
							name: 'estimatedItemHeight',
							type: 'number',
							description: 'Fallback height for unmeasured items in variable mode. Default: 40',
							required: false,
						},
						{
							name: 'stickyLast',
							type: 'boolean',
							description: 'Auto-scroll to bottom when new items are appended. Default: true',
							required: false,
						},
						{
							name: 'el',
							type: 'JSX.GlobalHTMLAttributes',
							description: 'Pass-through HTML attributes for the scroll container',
							required: false,
						},
					]}
				/>
