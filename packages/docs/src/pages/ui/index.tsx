...
export default function UIPage() {
  return (
    <article>
      ...
      <p>
        <code>@pounce/ui</code> is a component library built on <code>@pounce/core</code>.
        It uses the <strong>adapter pattern</strong> to decouple component logic from CSS
        framework styling — swap PicoCSS for any other framework without changing component code.
      </p>

      <Section title="Adapter Setup">
        <p>
          Install a CSS framework adapter before using components.
          The adapter provides variant classes, transitions, and component configs.
        </p>
      ...
      <Section title="Components">
        ...
        <table>
          ...
          <tbody>
            <tr><td><code>Button</code></td><td>Interactive button with variants, icons, loading state</td></tr>
            <tr><td><code>ButtonGroup</code></td><td>Group buttons with shared styling</td></tr>
            <tr><td><code>Accordion</code></td><td>Collapsible content section</td></tr>
            <tr><td><code>Card</code></td><td>Content container with header/footer slots</td></tr>
            <tr><td><code>Menu</code></td><td>Navigation menu with keyboard support</td></tr>
            <tr><td><code>Dialog</code></td><td>Modal dialog with backdrop and focus trap</td></tr>
            <tr><td><code>Toast</code></td><td>Notification toasts with variants</td></tr>
            <tr><td><code>Drawer</code></td><td>Slide-out panel (left/right)</td></tr>
            <tr><td><code>Progress</code></td><td>Progress bar with indeterminate mode</td></tr>
            <tr><td><code>InfiniteScroll</code></td><td>Virtualized list with variable height</td></tr>
            <tr><td><code>Toolbar</code></td><td>Horizontal toolbar with spacer</td></tr>
            <tr><td><code>Stars</code></td><td>Star rating input with single or range selection</td></tr>
      ...
      <Section title="Display Context">
        <p>
          <code>DisplayProvider</code> manages theme state and provides
          display context to all descendants. <code>ThemeToggle</code> switches
      ...
      <Section title="Directives">
        ...
        <ul>
          <li><code>use:badge</code> — badge overlay on any element</li>
          <li><code>use:intersect</code> — intersection observer</li>
          <li><code>use:loading</code> — loading state with spinner</li>
          <li><code>use:pointer</code> — pointer event tracking</li>
          <li><code>use:resize</code> — resize observer</li>
          <li><code>use:scroll</code> — scroll position tracking</li>
        </ul>
      </Section>
    </article>
  )
