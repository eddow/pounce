...
import { Accordion, AccordionGroup, Button, Select, Switch, Stack, Inline } from '@pounce/ui'
  ...
  return (
    <Stack gap="md">
      ...
      <Accordion summary={state.summary} open={state.open} variant={state.variant}>
        <p>This content is revealed when the accordion is open. The <code>open</code> prop is two-way bound — toggling the accordion updates the switch above.</p>
      </Accordion>
    </Stack>
  )
}
...
export default function AccordionPage() {
  return (
    <article>
      <h1>Accordion</h1>
      <p>
        Collapsible content section built on native <code>{'<details>'}</code>.
        Supports controlled state, variants, and auto-merging borders when stacked.
      </p>
      ...
      <Section title="Controlled">
        <p>
          <code>open={'{state.open}'}</code> is two-way bound — the accordion reads and writes it directly.
          No <code>onToggle</code> callback needed for basic state sync.
        </p>
        <Demo
        ...
        <Demo
          ...
          component={
            <AccordionGroup name="faq">
              <Accordion summary="Question 1"><p>Answer 1</p></Accordion>
              <Accordion summary="Question 2"><p>Answer 2</p></Accordion>
              <Accordion summary="Question 3"><p>Answer 3</p></Accordion>
            </AccordionGroup>
          }
        />
      </Section>
      ...
      <Section title="API Reference">
        ...
        <ApiTable props={[
          { name: 'summary', type: 'JSX.Children', description: 'Header content (always visible)', required: true },
          { name: 'open', type: 'boolean', description: 'Controlled open state — two-way bound. Default: false', required: false },
          { name: 'onToggle', type: '(open: boolean) => void', description: 'Side-effect callback when toggled (not needed for state sync)', required: false },
          ...
          { name: 'children', type: 'JSX.Children', description: 'Collapsible content', required: false },
        ]} />
        <h4>AccordionGroupProps</h4>
        <ApiTable props={[
          { name: 'name', type: 'string', description: 'Shared name for exclusive-open behaviour', required: true },
          { name: 'el', type: 'JSX.GlobalHTMLAttributes', description: 'Pass-through HTML attributes', required: false },
          { name: 'children', type: 'JSX.Children', description: 'Accordion children', required: false },
        ]} />
      </Section>
    </article>
  )
