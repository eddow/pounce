...
3. **Component-Level Rendering**:
   - `@pounce/board` uses `@pounce/core/server` utilities to render the matched route component into the HTML template.
   - It supports asynchronous data fetching via a **Reactive Rendering Pipeline**:
     ...
     4. As promises resolve, `@pounce/core` reactivity automatically updates the virtual DOM.
   ...
   - **Critical**: To ensure reactivity works correctly across module boundaries, the framework (`@pounce/core`, `@pounce/board`, `mutts`) must be loaded from the same Vite instance using `vite.ssrLoadModule`.
...
4. **Client Side (First Load)**:
   ...
   - The UI is hydrated by `@pounce/core` using the same component logic, but since the markup is already present, it attaches to the existing DOM nodes.
...
`@pounce/board` automatically integrates with `@pounce/pico`'s SSR styling system.
- Styles generated by `css` tagged templates (and `pure-glyf`) are collected during the server render.
- They are injected into the HTML `<head>` using `@pounce/pico/server`'s `getSSRStyles()`.
...
The most critical requirement for SSR in `@pounce/board` is ensuring that a **single instance** of the reactive framework (`mutts`, `@pounce/core`) is used throughout the request lifecycle.
