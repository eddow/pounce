...
test.describe('Blog App Consumer', () => {
	...
	test('GET / should return API info', async ({ request }) => {
		const res = await request.get('http://127.0.0.1:5276/')
	...
	test.describe('Posts API', () => {
		test('CRUD flow', async ({ request }) => {
			...
			const listRes = await request.get('http://127.0.0.1:5276/posts')
			...
			const createRes = await request.post('http://127.0.0.1:5276/posts', { data: newPost })
			...
			// 3. Get created post
			const getRes = await request.get(`http://127.0.0.1:5276/posts/${newId}`)
			expect(getRes.ok()).toBeTruthy()
			const fetchedPost = await getRes.json()
			expect(fetchedPost.content).toBe(newPost.content)

			// 4. Update post
			const updateRes = await request.put(`http://127.0.0.1:5276/posts/${newId}`, {
			...
			// 5. Delete post
			const deleteRes = await request.delete(`http://127.0.0.1:5276/posts/${newId}`)
			expect(deleteRes.ok()).toBeTruthy()

			// 6. Verify deletion
			const checkRes = await request.get(`http://127.0.0.1:5276/posts/${newId}`)
	...
	test.describe('Middleware', () => {
		...
		test('Posts middleware adds X-Resource header', async ({ request }) => {
			const res = await request.get('http://127.0.0.1:5276/posts')
	...
	test.describe('Route Groups', () => {
		test('GET /login (from (auth) group)', async ({ request }) => {
			// Test invalid login
			const res = await request.post('http://127.0.0.1:5276/login', {
				data: { username: 'user', password: 'wrong' },
			})
			expect(res.status()).toBe(401)

			// Test valid login
			const res2 = await request.post('http://127.0.0.1:5276/login', {
